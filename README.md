...
**받기:** [Releases](https://github.com/ssamssae/ipta/releases)에서 `Ipta-0.1.16-arm64.dmg`를 받습니다. 디스크를 열고 입타를 응용 프로그램 폴더로 끌어다 넣으면 됩니다.
...
8. 설정은 따로 된 창으로 열리고, 위쪽 막대를 잡고 옮길 수 있습니다. 말할 준비는 말한 소리를 글로 바꿀 파일이 이 맥에 있는지를 뜻합니다.
9. 말한 글을 다듬는 기능은 선택입니다. 받아 적기만 쓰면 입타만으로 됩니다. 그록·커서·클로드·코덱스로 다듬으려면 그 프로그램을 이 맥에 먼저 두고, 입타에서 본인 계정으로 로그인하세요. 프로그램이 없으면 설정의 **받는 곳 열기**를 누르면 안내 페이지가 열립니다. 요금은 그 계정으로 나갑니다. 키를 직접 넣을 수도 있습니다.
