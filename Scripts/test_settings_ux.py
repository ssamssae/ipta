...
def main() -> int:
    ...
    personalization = (ROOT / "App/Sources/PersonalizationView.swift").read_text(encoding="utf-8")
    section = (ROOT / "App/Sources/SettingsSection.swift").read_text(encoding="utf-8")
    failed = 0
    failed += check("no native GroupBox AX crash trigger", "GroupBox(" not in result + personalization + section)
    failed += check("no settings sheet", ".sheet(isPresented: $state.showSettings)" not in result)
    failed += check("status always visible", 'SettingsSection("지금 상태")' in result)
    failed += check("no collapsed details", "DisclosureGroup" not in result)
    failed += check("model box is plain language", 'SettingsSection("말할 준비")' in result)
